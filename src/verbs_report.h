...
std::vector<LangCount> computeLangCounts( const rw::IngestResult& ing )
{
    ...
    std::array<std::uint64_t, std::size_t( Lang::Haxe ) + 1> symbolTally {};  // sized on the LAST enum member
    for( const Symbol& s : ing.symbols )
    {
    ...
    std::array<std::uint64_t, std::size_t( Lang::Haxe ) + 1> fileTally {};    // sized on the LAST enum member
