...
// Haxe (20) is appended AFTER Lua for the SAME reason an EIGHTH time: it clamps into the identical
// Unknown-bucket headroom in serialize.h, zero renumbering of Cpp..Lua. A CODE language (classes,
// interfaces, enums, abstracts, typedefs, methods, calls, `extends`/`implements`, `import`/`using`).
enum class Lang : std::uint8_t { Cpp, Python, TypeScript, Go, Rust, Swift, ObjC, Markdown, JavaScript, Bash, Java, Ruby, Unknown, Json, CSharp, C, Toml, Yaml, Php, Lua, Haxe };

// short lang label — the terse XML/JSON attribute (lang="cpp|py|ts|go|rs|swift|objc|js|sh|java|rb|md|json|cs|c|toml|yaml|php|lua|hx").
...
// Essential-complexity coverage: 12 of the 16 CODE languages — every one EXCEPT Bash, PHP, Lua and Haxe.
...
// so the reading is "not measured", never "measured zero". Haxe (the 2026-09-06 port) is out for the
// same reason: its EIf/EFor/EWhile/switch_case/ETernary kinds and its per-catch `body:` fields ARE in
// the cx/ccx walk (test/haxecheck.sh pins the numbers), but its exit vocabulary (`throw` as an
// expression, `break`/`continue` inside `switch` arms that are NOT loops) was not measured, so ev stays
// withheld rather than guessed. Markdown/Json/Toml/
