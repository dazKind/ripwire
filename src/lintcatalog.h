...
inline constexpr std::array<Lang, 16> kCatalogLangs = { {
    ...
    // that claims to cover php/lua is backed by langOfPath knowing .php/.lua (lintrules.h). Haxe joins
    // on the same terms: naming family only, `.hx` known to langOfPath.
    Lang::Php, Lang::Lua, Lang::Haxe,
} };
