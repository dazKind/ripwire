...
first byte is markup still indexes), Lua, Haxe (`.hx` — tong/tree-sitter-haxe, chosen by measured
parse health over the more-starred alternative; see the CMakeLists.txt haxe block), Bash, Go, Rust,
Swift, C#, plus JSON, TOML and YAML configuration keys.
...
node in the `--deps`/`--arch` graph. **Haxe**: `using StringTools; s.trim()` is a static extension —
the typer rewrites the receiver, the text does not spell it — so the call is captured as `trim` and
resolves by name like any other method call while the `using` line is an import record, never a
receiver hint; enum-abstract values and non-SCREAMING fields are not indexed; there are no HAS-A field
edges. All three floors are asserted from the outside by `test/phpcheck.sh`, `test/luacheck.sh` and
`test/haxecheck.sh` so they stay decisions rather than drift.
